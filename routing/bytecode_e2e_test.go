...
func TestBytecodeRouterCfgE2EComplexRuleset(t *testing.T) {
	...
	if err := r.Attach(3, gonnect.NewLoopbackNetwork()); err != nil {
...
func TestBytecodeSplitRouterE2EDebugSystemDefaultMatcherMatchesNoFlows(
	t *testing.T,
) {
	router, err := NewBytecodeSplitRouter(SplitBytecodeRules{
		System: &sysnetdebug.System{},
...
func TestBytecodeSplitRouterE2EDebugSystemRuleMatcherErrorDropsRuleMatch(
	t *testing.T,
) {
	...
	system := &sysnetdebug.System{
		RuleMatcher: func(got sysnet.Rule, flow sockowner.FlowTuple) (bool, error) {
			...
			if flow.Proto != "tcp" || flow.LocalPort != 1 ||
				flow.RemotePort != 2 {
