...
//
// The package includes detached middleware, resolver adapters, an in-memory
// cache middleware, a simple UDP/TCP DNS client, and a simple UDP DNS server.
// These pieces are intentionally small and composable so callers can build
// chains or trees such as resolver adapters feeding shared cache middleware that
// fans out to local or remote DNS transports.
package dns
