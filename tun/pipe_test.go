...
func TestPipeBasic(t *testing.T) {
	...
	if name1 == name2 {
		t.Errorf("Expected different names, got %q and %q", name1, name2)
	}
...
func TestPipeBidirectional(t *testing.T) {
	...
	if sizes2[0] != len(data1) {
		t.Errorf("tun2.Read() size=%d, want %d", sizes2[0], len(data1))
	}
	if string(buf2[:sizes2[0]]) != string(data1) {
		t.Errorf("tun2.Read() data=%q, want %q", buf2[:sizes2[0]], data1)
	}
	...
	if sizes4[0] != len(data2) {
		t.Errorf("tun1.Read() size=%d, want %d", sizes4[0], len(data2))
	}
	if string(buf1[:sizes4[0]]) != string(data2) {
		t.Errorf("tun1.Read() data=%q, want %q", buf1[:sizes4[0]], data2)
	}
}
...
func TestPipeDifferentBatchSizes(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			bs1 := tun1.BatchSize()
			if bs1 != tc.batch {
				t.Errorf("tun1.BatchSize() = %d, want %d", bs1, tc.batch)
			}

			bs2 := tun2.BatchSize()
			if bs2 != tc.batch {
				t.Errorf("tun2.BatchSize() = %d, want %d", bs2, tc.batch)
			}
			...
			for i := range numPackets {
				...
				if sizes[0] != len(data) {
					t.Errorf("Read() size=%d, want %d", sizes[0], len(data))
				}
				if string(buf[:sizes[0]]) != string(data) {
					t.Errorf("Read() data=%q, want %q", buf[:sizes[0]], data)
				}
			}
		})
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if sizes[0] != packetSize {
				t.Errorf("Read() size=%d, want %d", sizes[0], packetSize)
			}
			if string(buf[:sizes[0]]) != string(data) {
				t.Errorf("Data mismatch: first %d bytes differ", sizes[0])
			}
		})
	}
}
...
func TestPipeBatchWithOffset(t *testing.T) {
	...
	for i := range numPackets {
		...
		if string(buf[:sizes[0]]) != string(expected) {
			t.Errorf("Packet %d: data=%q, want %q", i, buf[:sizes[0]], expected)
		}
	}
}
